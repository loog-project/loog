...
func (s *Store) AllResources() []*resource.ResourceData {
...
func (s *Store) StarredResources() []*resource.ResourceData {
...
func (s *Store) GetResource(uid string) *resource.ResourceData {
...
func (s *Store) AddWatchKind(rk resource.ResourceKind) []*resource.ResourceData {
	now := time.Now()
	var created []*resource.ResourceData
...
func (s *Store) ForEachResource(fn func(uid string, rd *resource.ResourceData)) {
	for uid, rd := range s.resources {
		fn(uid, rd)
	}
}
...
func (s *Store) GenerateRevision(rd *resource.ResourceData) resource.Revision {
	now := time.Now()
...
func generateDummyResourcesForKind(rk resource.ResourceKind, now time.Time) []dummyResourceSpec {
	ns := "default"
	if !rk.Namespaced {
		ns = ""
	}
