...
func New() *Store {
	...
	addResource("uid-pod-nginx-1", "Pod", "nginx-7d4f8b-abc12", "default", true, []resource.Revision{
		...
		{ID: 0x0003, PreviousID: 0x0001, EventType: resource.EventModified, Time: now.Add(-11 * time.Minute), Object: map[string]any{
			"apiVersion": "v1", "kind": "Pod",
			"metadata": map[string]any{"name": "nginx-7d4f8b-abc12", "namespace": "default", "uid": "uid-pod-nginx-1",
				"labels": map[string]any{"app": "nginx", "pod-template-hash": "7d4f8b"},
			},
		...
		{ID: 0x000a, PreviousID: 0x0003, EventType: resource.EventModified, Time: now.Add(-3 * time.Minute), Object: map[string]any{
			"apiVersion": "v1", "kind": "Pod",
			"metadata": map[string]any{"name": "nginx-7d4f8b-abc12", "namespace": "default", "uid": "uid-pod-nginx-1",
				"labels": map[string]any{"app": "nginx", "pod-template-hash": "7d4f8b"},
			},
	...
	addResource("uid-pod-nginx-2", "Pod", "nginx-7d4f8b-def34", "default", false, []resource.Revision{
		...
		{ID: 0x0004, PreviousID: 0x0002, EventType: resource.EventModified, Time: now.Add(-11 * time.Minute), Object: map[string]any{
			"apiVersion": "v1", "kind": "Pod",
			"metadata": map[string]any{"name": "nginx-7d4f8b-def34", "namespace": "default"},
			"spec":     map[string]any{"containers": []any{map[string]any{"name": "nginx", "image": "nginx:1.24"}}},
	...
	addResource("uid-pod-nginx-3", "Pod", "nginx-7d4f8b-ghi56", "default", false, []resource.Revision{
		{ID: 0x0005, EventType: resource.EventAdded, Time: now.Add(-10 * time.Minute), Object: map[string]any{
			...
			"spec":     map[string]any{"containers": []any{map[string]any{"name": "nginx", "image": "nginx:1.24", "resources": map[string]any{"limits": map[string]any{"memory": "64Mi"}}}}},
			"status":   map[string]any{"phase": "Running"},
		}},
		{ID: 0x0008, PreviousID: 0x0005, EventType: resource.EventModified, Time: now.Add(-7 * time.Minute), Object: map[string]any{
			"apiVersion": "v1", "kind": "Pod",
			"metadata": map[string]any{"name": "nginx-7d4f8b-ghi56", "namespace": "default"},
			"spec":     map[string]any{"containers": []any{map[string]any{"name": "nginx", "image": "nginx:1.24", "resources": map[string]any{"limits": map[string]any{"memory": "64Mi"}}}}},
		...
		{ID: 0x0009, PreviousID: 0x0008, EventType: resource.EventModified, Time: now.Add(-6 * time.Minute), Object: map[string]any{
			"apiVersion": "v1", "kind": "Pod",
			"metadata": map[string]any{"name": "nginx-7d4f8b-ghi56", "namespace": "default"},
			"spec":     map[string]any{"containers": []any{map[string]any{"name": "nginx", "image": "nginx:1.24", "resources": map[string]any{"limits": map[string]any{"memory": "64Mi"}}}}},
	...
	addResource("uid-deploy-nginx", "Deployment", "nginx-deployment", "default", true, []resource.Revision{
		{ID: 0x0006, EventType: resource.EventAdded, Time: now.Add(-13 * time.Minute), Object: map[string]any{
			...
			"spec":     map[string]any{"replicas": float64(1), "selector": map[string]any{"matchLabels": map[string]any{"app": "nginx"}}, "template": map[string]any{"spec": map[string]any{"containers": []any{map[string]any{"name": "nginx", "image": "nginx:1.24"}}}}},
			"status":   map[string]any{"readyReplicas": float64(0), "replicas": float64(1)},
		}},
		{ID: 0x0007, PreviousID: 0x0006, EventType: resource.EventModified, Time: now.Add(-11 * time.Minute), Object: map[string]any{
			"apiVersion": "apps/v1", "kind": "Deployment",
			"metadata": map[string]any{"name": "nginx-deployment", "namespace": "default"},
			"spec":     map[string]any{"replicas": float64(3), "selector": map[string]any{"matchLabels": map[string]any{"app": "nginx"}}, "template": map[string]any{"spec": map[string]any{"containers": []any{map[string]any{"name": "nginx", "image": "nginx:1.24"}}}}},
		...
		{ID: 0x000b, PreviousID: 0x0007, EventType: resource.EventModified, Time: now.Add(-3 * time.Minute), Object: map[string]any{
			"apiVersion": "apps/v1", "kind": "Deployment",
			"metadata": map[string]any{"name": "nginx-deployment", "namespace": "default"},
			"spec":     map[string]any{"replicas": float64(3), "selector": map[string]any{"matchLabels": map[string]any{"app": "nginx"}}, "template": map[string]any{"spec": map[string]any{"containers": []any{map[string]any{"name": "nginx", "image": "nginx:1.25"}}}}},
	...
	addResource("uid-svc-nginx", "Service", "nginx-svc", "default", false, []resource.Revision{
		...
		{ID: 0x000d, PreviousID: 0x000c, EventType: resource.EventModified, Time: now.Add(-2 * time.Minute), Object: map[string]any{
			"apiVersion": "v1", "kind": "Service",
			"metadata": map[string]any{"name": "nginx-svc", "namespace": "default"},
	...
	addResource("uid-cm-nginx", "ConfigMap", "nginx-config", "default", false, []resource.Revision{
		...
		{ID: 0x000f, PreviousID: 0x000e, EventType: resource.EventModified, Time: now.Add(-5 * time.Minute), Object: map[string]any{
			"apiVersion": "v1", "kind": "ConfigMap",
			"metadata": map[string]any{"name": "nginx-config", "namespace": "default"},
	...
	addResource("uid-pod-api", "Pod", "api-server-6b7c-xyz99", "default", false, []resource.Revision{
		...
		{ID: 0x0011, PreviousID: 0x0010, EventType: resource.EventModified, Time: now.Add(-9 * time.Minute), Object: map[string]any{
			"apiVersion": "v1", "kind": "Pod",
			"metadata": map[string]any{"name": "api-server-6b7c-xyz99", "namespace": "default", "labels": map[string]any{"app": "api-server"}},
			"spec":     map[string]any{"containers": []any{map[string]any{"name": "api", "image": "myapp/api:v2.1.0"}}},
		...
		}, Patch: map[string]any{"status": map[string]any{"containerStatuses": []any{map[string]any{"state": map[string]any{"waiting": map[string]any{"reason": "CrashLoopBackOff"}}, "restartCount": float64(3)}}}}},
		{ID: 0x0014, PreviousID: 0x0011, EventType: resource.EventModified, Time: now.Add(-4 * time.Minute), Object: map[string]any{
			"apiVersion": "v1", "kind": "Pod",
			"metadata": map[string]any{"name": "api-server-6b7c-xyz99", "namespace": "default", "labels": map[string]any{"app": "api-server"}},
	...
	addResource("uid-cm-coredns", "ConfigMap", "coredns-config", "kube-system", false, []resource.Revision{
		...
		{ID: 0x0017, PreviousID: 0x0016, EventType: resource.EventModified, Time: now.Add(-8 * time.Minute), Object: map[string]any{
			"apiVersion": "v1", "kind": "ConfigMap",
			"metadata": map[string]any{"name": "coredns-config", "namespace": "kube-system"},
	...
	addResource("uid-crd-myapp", "MyApp", "production", "operators", true, []resource.Revision{
		...
		{ID: 0x0025, PreviousID: 0x0020, EventType: resource.EventModified, Time: burstBase, Object: map[string]any{
			"apiVersion": "myapp.example.com/v1", "kind": "MyApp",
			"metadata": map[string]any{"name": "production", "namespace": "operators"},
		...
		{ID: 0x002a, PreviousID: 0x0025, EventType: resource.EventModified, Time: burstBase.Add(5 * time.Second), Object: map[string]any{
			"apiVersion": "myapp.example.com/v1", "kind": "MyApp",
			"metadata": map[string]any{"name": "production", "namespace": "operators"},
			"spec":     map[string]any{"replicas": float64(3), "image": "myapp/web:v1.1.0", "port": float64(3000)},
	...
	addResource("uid-deploy-myapp", "Deployment", "myapp-web", "operators", false, []resource.Revision{
		...
		{ID: 0x0029, PreviousID: 0x0026, EventType: resource.EventModified, Time: burstBase.Add(4 * time.Second), Object: map[string]any{
			"apiVersion": "apps/v1", "kind": "Deployment",
			"metadata": map[string]any{"name": "myapp-web", "namespace": "operators", "ownerReferences": []any{map[string]any{"kind": "MyApp", "name": "production"}}},
			"spec":     map[string]any{"replicas": float64(3), "template": map[string]any{"spec": map[string]any{"containers": []any{map[string]any{"name": "web", "image": "myapp/web:v1.1.0"}}}}},
			"status":   map[string]any{"readyReplicas": float64(3), "replicas": float64(3)},
